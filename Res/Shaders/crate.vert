...
void main()
{
	...
	vec3 T = normalize(vec3(u_modelToWorld * vec4(tangent, 0.f)));
	vec3 N = normalize(vec3(u_modelToWorld * vec4(normal, 0.f)));
	vec3 B = normalize(vec3(u_modelToWorld * vec4(bitangent, 0.f)));
	v2f_TBN = mat3(T, B, N);
